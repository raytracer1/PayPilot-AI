...
interface FlowchartContainerProps { path: PathOption; }

interface FlowNode { id: string; label: string; sublabel: string; phase: "start" | "on_ramp" | "network" | "off_ramp" | "end"; x: number; y: number; }

const PHASE_COLORS: Record<string, string> = { start: "#f59e0b", on_ramp: "#3b82f6", network: "#8b5cf6", off_ramp: "#10b981", end: "#f59e0b" };

export default function FlowchartContainer({ path }: FlowchartContainerProps) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < nodes.length - 1; i++) {
      ...
      g.append("line").attr("x1", from.x + 64).attr("y1", from.y).attr("x2", to.x - 64).attr("y2", to.y).attr("stroke", "#d1d5db").attr("stroke-width", 3).attr("stroke-dasharray", "8,4").attr("opacity", 0.6);
      const midX = (from.x + 64 + to.x - 64) / 2;
      g.append("polygon").attr("points", `${midX + 6},${from.y} ${midX - 4},${from.y - 5} ${midX - 4},${from.y + 5}`).attr("fill", "#9ca3af");
    }

    nodes.forEach((node) => {
      const ng = g.append("g").attr("transform", `translate(${node.x}, ${node.y})`);
      ng.append("circle").attr("r", 32).attr("fill", PHASE_COLORS[node.phase]).attr("opacity", 0.15);
      ng.append("circle").attr("r", 32).attr("fill", "none").attr("stroke", PHASE_COLORS[node.phase]).attr("stroke-width", 2.5);
      ng.append("text").attr("y", -8).attr("text-anchor", "middle").attr("font-size", 11).attr("font-weight", 600).attr("fill", PHASE_COLORS[node.phase]).text(node.label);
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 overflow-x-auto">
      <h4 className="font-semibold text-gray-900 dark:text-white mb-3 text-sm">Transfer Flow</h4>
      <svg ref={svgRef} width="100%" height="200" className="min-w-[600px]" />
    </div>
  );
}
